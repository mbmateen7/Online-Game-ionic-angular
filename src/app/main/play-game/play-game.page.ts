...
export class PlayGamePage implements OnInit {
        ...
        function btnUpdate(that) {
            ...
            if (THIS.countWord) {
                ...
                for (let index = 0; index < THIS.gameObj.word.length; index++) {
                    if (
                        THIS.gameObj.word[index].toLowerCase() ==
                        that.innerHTML.toLowerCase()
                    ) {
                        THIS.lettersArr.controls[index].setValue(
                            that.innerHTML
                        );
                    }
    ...
    revertButtons(cancel = 'no') {
        if (cancel == 'cancel') {
            this.secondaryValues.ranges[
                this.secondaryFilterSelected
            ].sliderValue = this.lastValue;
    ...
    getOwnedItemList() {
        this.restService
            ...
            .subscribe((res: any) => {
                ...
                this.primayFilterRemovalList = this.ownedItemsList.filter(
                    (x) => x.shop_id == 3
                );
                this.secondaryFilterRemovalList = this.ownedItemsList.filter(
                    (x) => x.shop_id == 4
                );
                this.addTurn = this.ownedItemsList.filter(
                    (x) => x.shop_id == 5
                );
                this.revealList = this.ownedItemsList.filter(
                    (x) => x.shop_id == 6
                );
            });
    }
    ...
    onremovalPrimaryFilter() {
        ...
        if (this.primayFilterRemovalList.length > 0) {
            this.itemUsageConsentDialog(
                'RemovePrimary',
                'It will remove the primary filter from puzzle'
            )
                .then((res: any) => {
                    if (res.isConfirmed) {
                        this.primaryFilter = '';
                        ...
                        this.restService
                            .postRequestToken('shop/update-item', {
                                user_shop_id:
                                    this.primayFilterRemovalList[0].id,
                            })
                            .subscribe((res: any) => {
                                if (res.status) {
                                    Swal.fire({
    ...
    onremovalSecondaryFilter() {
        ...
        if (this.secondaryFilterRemovalList.length > 0) {
            this.itemUsageConsentDialog(
                'RemoveSecondary',
                'It will remove the secondary filter from the puzzle'
            )
                .then((res) => {})
                .catch((err) => {
                    console.log('Error', err);
                ...
                .then((res: any) => {
                    if (res.isConfirmed) {
                        ...
                        for (let i = 0; i < this.revealCount; i++) {
                            this.lettersArr.controls[i].setValue(
                                this.gameObj.word[i]
                            );
                        }
                        this.restService
